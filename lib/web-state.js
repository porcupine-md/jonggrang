...
function deriveState(projectPath) {
  // Draft state: any pending draft session under .drafts/
  const draftsDir = path.join(projectPath, '.jonggrang', '.drafts');
  if (fs.existsSync(draftsDir)) {
    try {
      const sessions = fs.readdirSync(draftsDir)
        .map(name => {
          const p = path.join(draftsDir, name, 'plan.md');
          try { return { name, mtime: fs.statSync(p).mtimeMs }; } catch { return null; }
        })
        .filter(Boolean)
        .sort((a, b) => b.mtime - a.mtime);
      if (sessions.length > 0) {
        return { state: 'draft', planMtime: sessions[0].mtime, draftCount: sessions.length };
      }
    } catch (err) {
      return { state: 'error', reason: 'corrupt_draft_dir', message: err.message };
    }
  }
