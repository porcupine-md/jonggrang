...
function mountDevice(device, workdir) {
  ...
  if (isMounted(mountPoint)) return { mountPoint, mounted: false };
...
function unmountDevice(device, workdir) {
  ...
  for (const argv of [['fusermount', '-u', mountPoint], ['umount', mountPoint]]) {
...
module.exports = {
  ...
  mountDevice, unmountDevice, isMounted, mountPointFor,
