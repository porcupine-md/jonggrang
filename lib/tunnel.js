...
function reservePort(registry, exceptId = null) {
  ...
  for (let p = PORT_MIN; p <= PORT_MAX; p++) {
    // Free in the registry is not the same as free on the machine. A tunnel from a
    // device that was removed keeps its port until its ssh client dies, and handing
    // that port to a new device gives it a listener it does not own: the dashboard
    // reads the port as listening and calls the new device online, while its own
    // `ssh -R` is refused for a port already taken. Registry-only bookkeeping
    // reported a device as reachable when what answered was somebody else's tunnel.
    if (!taken.has(p) && !portInUseSync(p)) return p;
  }
  throw new Error(`no free tunnel port in ${PORT_MIN}-${PORT_MAX} (${taken.size} devices registered)`);
}

/**
 * Is anything listening on this loopback port, right now, without waiting?
 *
 * provisionDevice is synchronous all the way up to the CLI and the HTTP handler,
 * so the async probe (portListening) cannot be used here. On Linux the kernel
 * already has the answer in /proc. Anywhere else this says "cannot tell" — it is a
 * guard against a stale listener, not the thing that keeps devices apart, so
 * failing open leaves behaviour exactly as it was.
 */
function portInUseSync(port) {
  ...
  for (const file of ['/proc/net/tcp', '/proc/net/tcp6']) {
    ...
    for (const line of text.split('\n').slice(1)) {
      ...
      // local_address is `<hex ip>:<hex port>`; 0A is TCP_LISTEN.
 ...
 */
/**
 * A floor under every writer of this file: one entry is one line. The builders
 * above already normalise the key, so reaching this means a caller assembled an
 * entry some other way — and appending it would silently authorize whatever came
 * after the newline.
 */
function assertSingleLine(entry) {
  if (/[\r\n]/.test(String(entry))) {
    throw new Error('an authorized_keys entry cannot span lines');
  }
}
...
function agentKeyEntry(pubkey, wrapper) {
  ...
  // One line, same reason as tunnelKeyEntry: this one arrives from the server over
  // ssh (or, now, from a wizard the user copied it into).
  const body = keyBody(pubkey);
  if (!body) throw new Error('not an ssh public key');
  return `restrict,pty${forced} ${body}`;
}
...
/**
 * The fingerprint ssh itself would print for this key — the thing a person can
 * compare against what the other machine says. No subprocess: it is a sha256 of
 * the decoded blob, which is all `ssh-keygen -lf` does for the SHA256 form.
 */
function keyFingerprint(pubkey) {
  const body = keyBody(pubkey);
  if (!body) return '';
  try {
    const blob = Buffer.from(body.split(' ')[1], 'base64');
    return `SHA256:${crypto.createHash('sha256').update(blob).digest('base64').replace(/=+$/, '')}`;
  } catch { return ''; }
}

/**
 * Same check, but it says what was wrong. keyBody returning null is enough for a
 * caller that already knows the input is a key; a paste box is not that caller.
 *
 * Returns { body } or { error }. `body` is type + base64 with no comment: the
 * comment is the user's own text and nothing downstream needs it, so it is
 * dropped rather than carried into a file where a newline in it would matter.
 */
function validatePubkey(text) {
  const raw = String(text || '').trim();
  if (!raw) return { error: 'Paste the device\'s public key.' };
  if (/-----BEGIN/.test(raw)) {
    return { error: 'That is a PRIVATE key. Paste the .pub file — the public half — instead.' };
  }
  const lines = raw.split(/\r?\n/).filter(l => l.trim());
  if (lines.length > 1) {
    return { error: `Paste one key, not ${lines.length}. authorized_keys is line-based, so a second line would be a second key.` };
  }
  const body = keyBody(raw);
  if (!body) {
    return { error: 'Not an ssh public key. Expected a line like `ssh-ed25519 AAAAC3Nz… user@host`.' };
  }
  return { body };
}
...
function tunnelKeyEntry(pubkey, port) {
  // keyBody, not pubkey.trim(): a key with a newline in it used to become TWO
  // authorized_keys lines, and only the first one carried these restrictions.
  // The second was whatever the text after the newline said — an unrestricted key,
  // if that is what was pasted. Reachable from the CLI by someone who already had
  // a shell here; reachable from a browser once a form accepts a pasted key.
  const body = keyBody(pubkey);
  if (!body) throw new Error('not an ssh public key');
...
function provisionDevice({ id, label, pubkey, localuser, workdir, owner, platform, rotate }) {
  ...
  // lives on its authorized_keys LINE, and that file is keyed by the key: a second
  // device sharing a key gets no line of its own, so it is either refused by
  // `permitlisten` or — worse — allowed onto the first device's port. Refusing here
  // is the only outcome that is not a surprise later.
  const body = keyBody(pubkey);
  const clash = Object.entries(registry.devices)
    .find(([otherId, d]) => otherId !== deviceId && keyBody(d.pubkey) === body);
  if (clash) {
    const err = new Error(`That key is already registered to ${clash[1].label || clash[0]} (${clash[0]}). A device's key is how the server tells it apart, so each one needs its own.`);
    err.code = 'PUBKEY_IN_USE';
    err.device_id = clash[0];
    throw err;
  }
...
/**
 * Forget a device — and let go of it first.
 *
 * Dropping the registry entry alone leaves this server holding mounts of a
 * machine it no longer knows: they answer EIO once the tunnel goes, and the paths
 * stay occupied, so re-registering the same device cannot mount where it did.
 * The mounts are the server's own state, so they go here.
 *
 * What lives ON the device — its worktrees, its `~/.jonggrang/device.json`, the
 * server's key in its authorized_keys — is not this command's to delete. It is
 * returned instead, so the caller can say where it is.
 */
function removeDevice(id) {
  ...
  const released = releaseDeviceMounts(device);

  // And the key it opened the tunnel with. This file is the SERVER's, so leaving
  // the entry meant a removed device kept a working tunnel until someone edited
  // authorized_keys by hand — and its port kept answering, which a later device
  // could then be handed. The first version of this said "its key stays, remove it
  // by hand", conflating two different keys: the server's key ON THE DEVICE is the
  // one that is not ours to delete. This one is.
  const revoked = removeAuthorizedKey(device.pubkey);

  delete registry.devices[id];
  writeRegistry(registry);
  return { removed: true, unmounted: released, tunnel_key_revoked: revoked, device_side: deviceSideLeftovers(device, id) };
}

/** Every mount this server holds of one device, by the port only it uses. */
function deviceMountPoints(device) {
  const res = spawnSync('ps', ['-eo', 'args='], { encoding: 'utf8' });
  if (res.error || !res.stdout) return [];
  const points = [];
  for (const line of res.stdout.split('\n')) {
    if (!line.startsWith('sshfs ') || !line.includes(`-p ${device.port} `)) continue;
...
function releaseDeviceMounts(device) {
  const released = [];
  for (const point of deviceMountPoints(device)) {
    if (unmountDevice(device, point)) released.push(point);
  }
  return released;
}

/**
 * What stays on the developer's machine, for the caller to report. The registry
 * keys entries by id and does not repeat it inside them, so it arrives separately.
 */
function deviceSideLeftovers(device, id) {
  return {
    worktrees: path.posix.dirname(deviceWorktreePath({ ...device, id }, '_')),
    registration: '~/.jonggrang/device.json',
    authorized_key: `~/.ssh/authorized_keys (comment ${AGENT_KEY_COMMENT})`,
  };
}
...
/**
 * The bundle, guaranteed before anything points `--settings` at it.
 *
 * A project imported before the bundle moved into `.jonggrang-device/` — or one
 * whose bundle dir was deleted — is otherwise handed a settings path to a file
 * that is not there. Claude Code carries on without the hook, and the agent's
 * commands run HERE, on the server, against a project whose code is on the
 * device: the split view, arriving as silence rather than an error. The check is
 * the only thing between a missing file and that.
 *
 * Default home is this package's own root, which is where the source bundle
 * lives for every install shape jonggrang supports.
 */
function ensureDeviceHooks(projectPath, jonggrangHome = path.join(__dirname, '..')) {
  const settings = deviceSettingsPath(projectPath);
  const hook = path.join(deviceBundleDir(projectPath), 'redirect-bash.sh');
  if (fs.existsSync(settings) && fs.existsSync(hook)) return settings;
  installDeviceHooks(projectPath, jonggrangHome);
  return settings;
}
...
module.exports = {
  PORT_MIN, PORT_MAX,
  // server
  registryPath, readRegistry, writeRegistry, reservePort, portInUseSync,
  serverKeyPath, serverKeyFor, ensureServerKey, rotateDeviceKey,
  provisionDevice, listDevices, listDevicesLive, removeDevice, deviceMountPoints, touchDevice,
  buildSshExecArgs, deviceFor, validatePubkey, assertSingleLine, keyFingerprint, deviceRedirectEnv, installDeviceHooks, ensureDeviceHooks,
