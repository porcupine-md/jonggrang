 ...
 * tunnel, nothing else. The plan defers hardening of the server→device
 * direction, but this direction costs one option string, so there is no reason
 * to hand out a shell. `permitlisten` needs OpenSSH 7.5+ (2017).
 */
function tunnelKeyEntry(pubkey, port) {
  return `restrict,port-forwarding,permitlisten="localhost:${port}" ${pubkey.trim()}`;
}
...
/**
 * The key this machine opens the tunnel with. Prefers an existing personal key
 * — the developer has usually already authorized it on the server — and only
 * generates a jonggrang-specific one when there is nothing to reuse.
 */
function ensureDeviceKey() {
  ...
  for (const name of ['id_ed25519', 'id_rsa']) {
    ...
    if (fs.existsSync(p) && fs.existsSync(`${p}.pub`)) {
      return { path: p, pub: fs.readFileSync(`${p}.pub`, 'utf8').trim(), generated: false };
    }
  }
 ...
 */
function requestProvision({ server, pubkey, label, localuser, workdir, id, sshArgs = [] }) {
  const argv = [
    ...sshArgs, server,
    'jonggrang', 'device', 'provision',
...
function tunnelArgv(cfg, { useAutossh = hasAutossh() } = {}) {
  const ssh = [
    ...
    '-R', `${cfg.port}:localhost:${cfg.local_ssh_port || 22}`,
    cfg.server,
  ];
  return useAutossh
