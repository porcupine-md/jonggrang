 ...
 *
 * What remains is §7's real exposure: commands as that user. The answer to THAT
 * is a dedicated device account (`device register --user jonggrang-agent`), not
 * an option string — see the warning at registration.
 */
function agentKeyEntry(pubkey) {
  return `restrict,pty ${pubkey.trim()}`;
}
 ...
 */
function provisionDevice({ id, label, pubkey, localuser, workdir, owner, platform }) {
...
function deviceRedirectEnv(device, workdir) {
  return {
    ...
    JONGGRANG_DEVICE_KEY: serverKeyPath(),
  };
}

/**
 * Install the server-side redirect bundle into a device project.
 *
 * Kept separate from the `jonggrang init` bundle on purpose (§4): that one also
 * lands on the device, and a redirect hook there would ssh a command to the very
 * machine already running it. This bundle exists only in the SERVER's copy of a
 * device project.
 */
function installDeviceHooks(projectPath, jonggrangHome) {
  ...
  const hooksDir = path.join(projectPath, 'hooks', 'device');
  fs.mkdirSync(hooksDir, { recursive: true });
  const dst = path.join(hooksDir, 'redirect-bash.sh');
  ...
  let settings = {};
  try { settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8')); } catch { /* first write */ }
  settings.hooks = settings.hooks || {};
  const pre = settings.hooks.PreToolUse = settings.hooks.PreToolUse || [];
  const command = `bash "${dst}"`;
  const already = pre.some(m => (m.hooks || []).some(h => h.command === command));
  if (!already) pre.push({ matcher: 'Bash', hooks: [{ type: 'command', command }] });
  fs.writeFileSync(settingsPath, `${JSON.stringify(settings, null, 2)}\n`);
  return { hook: dst, settings: settingsPath, installed: !already };
}
...
function mountDevice(device, workdir) {
  ...
  return { mountPoint, mounted: true };
}
...
function deviceWorktreePath(device, featureId) {
  return `/tmp/jonggrang-worktrees/${device.label.replace(/[^A-Za-z0-9._-]/g, '_')}/${featureId}`;
}
 ...
 */
function requestProvision({ server, pubkey, label, localuser, workdir, id, sshArgs = [], remoteBin, platform }) {
...
module.exports = {
  ...
  portListening,
  // keys
  authorizedKeysPath, addAuthorizedKey, setAuthorizedKey, keyBody, tunnelKeyEntry, agentKeyEntry,
  // local
