...
function listBaselinePacks(catalogDir = baselineCatalogPath()) {
  ...
  for (const pack of packs) {
    if (seenKeys.has(pack.key)) duplicateKeys.add(pack.key);
    seenKeys.add(pack.key);
  }
  for (const pack of packs) {
    if (!duplicateKeys.has(pack.key)) continue;
    pack.valid = false;
    pack.errors = [...pack.errors, `duplicate baseline key: ${pack.key}`];
  }
...
function validateBaselineManifest(manifest, packDir) {
  ...
  for (const field of ['recommend_keywords', 'recommend_frameworks']) {
    if (manifest[field] !== undefined && (!Array.isArray(manifest[field]) || manifest[field].some(value => typeof value !== 'string' || !value.trim()))) {
      errors.push(`${field} must be an array of non-empty strings when present`);
    }
  }
  if (manifest.recommend_priority !== undefined && (!Number.isFinite(manifest.recommend_priority) || manifest.recommend_priority < 0)) {
    errors.push('recommend_priority must be a non-negative number when present');
  }
 ...
 */
/**
 * Which UI task owns a planned token source.
 *
 * `token_status: planned` needs exactly one foundation task — every other UI task
 * is made to depend on it. The decomposition marks that flag by instruction, not
 * by construction, so it sometimes marks none and sometimes several. Approval used
 * to refuse the whole run there: tasks written, then rolled back, the plan draft
 * left untouched, and the same plan approving fine on the next attempt. A coin
 * flip, reported to the user as "no new tasks were created".
 *
 * So choose, and be able to say why: the task the decomposition marked when it
 * marked one, the first of them when it marked several, and otherwise the first UI
 * task — which is the one the others are ordered behind anyway.
 *
 * Returns { id, reason } where reason is null when the decomposition decided.
 */
function chooseTokenOwner(uiTasks) {
  const tasks = Array.isArray(uiTasks) ? uiTasks : [];
  if (tasks.length === 0) return null;
  const marked = tasks.filter(task => task.ui_context && task.ui_context.foundation === true);
  if (marked.length === 1) return { id: marked[0].id, reason: null };
  if (marked.length > 1) {
    return { id: marked[0].id, reason: `the decomposition marked ${marked.length} UI-foundation tasks` };
  }
  return { id: tasks[0].id, reason: 'the decomposition marked no UI-foundation task' };
}
...
function auditUiProject(projectRoot, opts = {}) {
  ...
  return {
    ...
    baseline_packs: baselineKeys(),
    truncated: files.length >= (opts.maxFiles || 4000),
  };
}

function findExplicitBaseline(description, packs = validBaselinePacks()) {
  const text = String(description || '').toLowerCase();
  return packs.find(pack => text.includes(pack.key.toLowerCase()))?.key || null;
}

function recommendBaseline(description, audit = {}, packs = validBaselinePacks()) {
  const explicit = findExplicitBaseline(description, packs);
  ...
  const text = String(description || '').toLowerCase();
  const frameworks = (audit.framework || []).map(item => String(item).toLowerCase());
  return packs
    .filter(pack => {
      const keywords = pack.recommend_keywords || [];
      const recommendedFrameworks = pack.recommend_frameworks || [];
      return keywords.some(keyword => text.includes(keyword.toLowerCase()))
        || recommendedFrameworks.some(framework => frameworks.includes(framework.toLowerCase()));
    })
    .sort((a, b) => Number(b.recommend_priority || 0) - Number(a.recommend_priority || 0) || a.key.localeCompare(b.key))[0]?.key || null;
}
...
function validateUiGuide(projectRoot, guideOrPath, opts = {}) {
  ...
  else if (baseline !== 'existing-project' && !isBaselineKey(baseline)) errors.push(`unknown baseline: ${baseline}`);
  const parsed = markdownSections(content);
  ...
  for (const sectionName of ['Source map', 'Components and layout patterns']) {
    ...
    for (const reference of references) {
      ...
      // A guide for a new feature legitimately names planned component/section
      // locations that do not exist yet. Surface these as warnings, not blocking
      // errors — the load-bearing token_source is still hard-validated above.
      if (status !== 'ready' && !isPlannedToken) warnings.push(`referenced path does not exist: ${clean}`);
    }
  }
...
function buildUiPreferenceQuestion(planning) {
  const availableBaselines = planning.availableBaselines || baselineKeys();
  const recommended = availableBaselines.includes(planning.baseline) ? [planning.baseline] : [];
  const choices = recommended.length > 0 ? recommended : availableBaselines;
  return {
    id: 'ui-preference',
...
function resolveUiPreference(planning, answers) {
  ...
  if (value.startsWith('use:')) {
    ...
    if (!(planning.availableBaselines || baselineKeys()).includes(selected)) throw new Error(`unknown selected UI baseline: ${selected}`);
...
function buildPlanningContext(projectRoot, sessionId, description, audit, chosenBaseline = null) {
  ...
  const baselinePack = availableBaselines.includes(baseline) ? loadBaselinePack(baseline) : null;
...
3. ${guideStatus === 'unchanged' ? `Use the existing \`${existingGuide}\`; do not rewrite it merely to restyle the project.` : `Write the proposed complete project guide to \`${guideDraft}\`. It must use format \`${GUIDE_FORMAT}\`, pin the selected baseline, name a canonical token source and status, and contain all eight required sections. Use these EXACT section titles verbatim as \`##\` level-2 headings, with no numbering, rewording, additions, or omissions:\n${REQUIRED_GUIDE_SECTIONS.map(s => '   - ' + s).join('\n')} In the Source map and Components sections, every file path you cite must already exist and be written exactly as a project-relative path (for example src/App.jsx, not App.jsx); use the word none for resources that do not exist, and never invent, abbreviate, or guess a path.`}
