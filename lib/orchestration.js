...
// The last phase a compact run executes: Implement. Everything after it is a
// quality gate, which compact mode defers (see finalizeRemainingPhases).
const COMPACT_LAST_PHASE = 8;
 ...
 */
function createManifest(projectRoot, featureId, description, workType, options = {}) {
  const activePhases = getActivePhases(workType);
  const manifest = {
    feature_id: featureId,
    description,
    work_type: workType,
    // 'full' runs the whole pipeline; 'compact' stops after Implement and defers
    // the quality gates (they can still be resumed later).
    pipeline_mode: options.pipelineMode === 'compact' ? 'compact' : 'full',
...
/**
 * Close out a run that will not execute its remaining phases — compact mode
 * (stop after Implement) and the BUGFIX quality-gate skip both end here.
 *
 * Every active phase that has not completed is marked `skipped` with a reason,
 * and the run itself is marked completed so the pipeline view shows a terminal
 * state instead of hanging at Implement. Idempotent.
 *
 * @returns {{manifest: object, skipped: number[]}|null} null if no manifest.
 */
function finalizeRemainingPhases(manifestPath, { reason = 'skipped' } = {}) {
  const manifest = readManifest(manifestPath);
  if (!manifest) return null;

  const remaining = (manifest.active_phases || []).filter(n => {
    const s = manifest.phases?.[n]?.status;
    return s !== 'completed' && s !== 'skipped';
  });

  const now = new Date().toISOString();
  for (const n of remaining) {
    if (!manifest.phases?.[n]) continue;
    manifest.phases[n].status = 'skipped';
    manifest.phases[n].skip_reason = reason;
    manifest.phases[n].completed_at = now;
  }
  ...
  writeManifest(manifestPath, manifest);
  return { manifest, skipped: remaining };
}

/**
 * Re-open the phases a compact run skipped, so `work --resume` can execute the
 * quality gates later. Only phases carrying the compact skip reason are
 * reopened — a BUGFIX that deliberately skipped its gates stays closed.
 *
 * @returns {number[]} the phase numbers reopened (empty if there was nothing).
 */
function reopenCompactPhases(manifestPath) {
  const manifest = readManifest(manifestPath);
  if (!manifest) return [];

  const reopened = (manifest.active_phases || []).filter(n =>
    manifest.phases?.[n]?.status === 'skipped' &&
    manifest.phases[n].skip_reason === COMPACT_SKIP_REASON);
  if (reopened.length === 0) return [];

  for (const n of reopened) {
    manifest.phases[n].status = 'pending';
    manifest.phases[n].completed_at = null;
    delete manifest.phases[n].skip_reason;
  }
  manifest.status = 'in_progress';
  manifest.current_phase = reopened[0];
...
/**
 * Find the most recent compact-finalized run — a completed manifest whose gates
 * were skipped by compact mode. `work --resume` falls back to this when there is
 * no incomplete manifest, so a compact run stays resumable.
 */
function findCompactManifest(projectRoot) {
  return listManifests(projectRoot)
    .filter(e => e.manifest.pipeline_mode === 'compact' &&
      (e.manifest.active_phases || []).some(n =>
        e.manifest.phases?.[n]?.status === 'skipped' &&
        e.manifest.phases[n].skip_reason === COMPACT_SKIP_REASON))
    .sort((a, b) => new Date(b.manifest.updated_at) - new Date(a.manifest.updated_at))[0] || null;
}

/** Record which pipeline mode a run is executing ('full' | 'compact'). */
function setPipelineMode(manifestPath, mode) {
  const manifest = readManifest(manifestPath);
  if (!manifest) return null;
  if (manifest.pipeline_mode === mode) return manifest;
  manifest.pipeline_mode = mode;
  manifest.updated_at = new Date().toISOString();
  writeManifest(manifestPath, manifest);
  return manifest;
}
...
function buildPhaseContext(manifest, currentPhaseNum, projectRoot) {
  ...
  let uiBlock = '';
  if (projectRoot && currentPhaseNum >= 8) {
    try {
      const handoffPath = uiContext.featureHandoffPath(projectRoot, manifest.feature_id || manifest.featureId);
      if (fs.existsSync(handoffPath)) {
        const handoff = fs.readFileSync(handoffPath, 'utf8');
        const selected = ['Feature intent', 'Shared direction', 'References']
          .map(section => uiContext.extractMarkdownSection(handoff, section))
          .filter(Boolean)
          .join('\n\n');
        uiBlock = `\n\n## Feature UI Context\n\n${selected}\n\nRead only root-guide sections referenced by the handoff. Report \`UI_GUIDE_DRIFT\` when the guide, handoff, and code conflict.`;
      }
    } catch { /* best-effort — UI context optional */ }
  }
