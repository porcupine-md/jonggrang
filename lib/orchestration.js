...
/**
 * Register an agent run in manifest.
 */
function registerAgent(manifestPath, agentId, role, outputPath = null) {
  const manifest = readManifest(manifestPath);
  if (!manifest) return;
...
/**
 * Mark agent as done in manifest.
 */
...
function findIncompleteManifest(projectRoot) {
  ...
  const entries = fs.readdirSync(outputDir)
    ...
    .filter(Boolean)
    .filter(e => ['running', 'in_progress', 'paused'].includes(e.manifest.status))
