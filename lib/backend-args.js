...
function buildAgentArgs({ tool, model, effort }) {
  ...
  switch (tool) {
      ...
      if (model) {
        if (!model.includes('/')) {
          throw new Error(
            ...
            `Got: "${model}". Use --tool claude or --tool pi for bare model names.`
          );
        }
    ...
    case 'pi':
      ...
      // --effort → --thinking for Pi CLI
      // Valid thinking levels: off, minimal, low, medium, high, xhigh
      if (model) flags.push('--model', model);
      if (effort) flags.push('--thinking', effort);
      break;

    case 'jonggrang':
      // Pi SDK backend — model is selected via `jonggrang model` command and stored
      // as provider + model in jonggrang.json. CLI --model/--effort are not forwarded
      // to the SDK API; use `jonggrang model` to switch models for this backend.
      break;
