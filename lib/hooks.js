...
function generateOpenCodePlugin(_projectRoot) {
  // Uses npm package resolution and runtime __dirname — no hardcoded absolute paths.
  return [
    `// Jonggrang orchestration plugin for OpenCode`,
    `// Generated by: jonggrang init — do not edit manually`,
    `const { createPlugin } = require('jonggrang/hooks/opencode/plugin');`,
...
function installOpenCodePlugin(projectRoot, jonggrangInstallDir) {
  ...
  // Write plugin entry point using npm package resolution (no hardcoded absolute paths).
  // - require('jonggrang/hooks/opencode/plugin') resolves via node_modules regardless of install location
  // - __dirname resolves to {project}/.opencode/plugins/ at runtime, so ../../ = project root
