...
// and registers it in ~/.jonggrang/agent/settings.json
// ============================================================

/**
 * Install Jonggrang Pi extension in project.
 * Copies hooks/pi/jonggrang-extension.ts → .jonggrang/extensions/jonggrang.ts
 * and registers the extensions dir in ~/.jonggrang/agent/settings.json.
 */
function installPiExtension(projectRoot, jonggrangInstallDir) {
  ...
  // Register extension path in Pi's settings.json.
  // Prefer ~/.pi/agent/settings.json if Pi is already installed, else use ~/.jonggrang/agent.
  const os = require('os');
  ...
  const existingExts = Array.isArray(settings.extensions) ? settings.extensions : [];
  if (!existingExts.includes(extensionsDir)) {
    settings.extensions = [...existingExts, extensionsDir];
  }
