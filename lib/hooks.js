...
function installCodexHooks(projectRoot, jonggrangInstallDir) {
  ...
  // Ensure hooks/codex/ exists in the project (copyDirRecursive in
  // installHooksForTool usually handles this, but guard for standalone calls).
  const destCodexHooks = path.join(projectRoot, 'hooks', 'codex');
  const srcCodexHooks = path.join(jonggrangInstallDir || '', 'hooks', 'codex');
  if (jonggrangInstallDir && fs.existsSync(srcCodexHooks) && !fs.existsSync(destCodexHooks)) {
