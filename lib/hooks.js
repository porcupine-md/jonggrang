...
// Installs .codex/hooks.json so codex CLI loads jonggrang enforcement
// via its native hooks API (PreToolUse / PostToolUse / Stop / SubagentStop /
// SubagentStart / SessionStart). See docs/AGENTTOOLS.md.
// ============================================================

/**
 * Install Codex hooks in project.
 * Copies hooks/codex/* → <project>/hooks/codex/ (already done by copyDirRecursive
 * in installHooksForTool, but re-run here for the standalone-install case) and
 * writes .codex/hooks.json pointing at the dispatcher.
 */
function installCodexHooks(projectRoot, jonggrangInstallDir) {
  const codexDir = path.join(projectRoot, '.codex');
  fs.mkdirSync(codexDir, { recursive: true });

  // Keep hooks/codex/ fresh for standalone reinstalls too. installHooksForTool
  // already copies the whole hooks tree, but installCodexHooks can be called
  // directly; always copy/overwrite rather than leaving stale dispatcher code.
  const destCodexHooks = path.join(projectRoot, 'hooks', 'codex');
  const srcCodexHooks = path.join(jonggrangInstallDir || '', 'hooks', 'codex');
  if (jonggrangInstallDir && fs.existsSync(srcCodexHooks)) {
    copyDirRecursive(srcCodexHooks, destCodexHooks);
  }

  // Write .codex/hooks.json from the template, stripping the _comment / _codex_events
  // helper keys so codex only sees the real "hooks" object.
  // Template lives at <installDir>/hooks/codex/hooks.json (source) or
  // <projectRoot>/hooks/codex/hooks.json (after copyDirRecursive ran).
  const templatePath = path.join(jonggrangInstallDir || projectRoot, 'hooks', 'codex', 'hooks.json');
  let raw;
  try {
    raw = JSON.parse(fs.readFileSync(templatePath, 'utf8'));
  } catch (e) {
    // Fail LOUD, not silent. Writing {hooks:{}} + installed:true here would
    // silently recreate issue #68 (valid codex backend, zero enforcement).
    throw new Error(
      `installCodexHooks: cannot read codex hooks template at ${templatePath}: ${e.message}`
    );
  }
  if (!raw.hooks || typeof raw.hooks !== 'object') {
    throw new Error(
      `installCodexHooks: codex hooks template at ${templatePath} has no "hooks" object`
    );
  }
  // Sanity-check expected lifecycle events are present + non-empty so a
  // half-broken template never silently installs a no-op config.
  const missing = CODEX_REQUIRED_EVENTS.filter(
    (eventName) => !Array.isArray(raw.hooks[eventName]) || raw.hooks[eventName].length === 0
  );
  if (missing.length) {
    throw new Error(
      `installCodexHooks: template at ${templatePath} is missing non-empty hooks for events: ${missing.join(', ')}`
    );
  }
  const hooksConfig = { hooks: raw.hooks };
...
/**
 * Install all backend hook assets during `jonggrang init`.
 * The selected tool is kept as a reserved argument for API compatibility, but
 * installing every backend lets projects switch tools without re-running init.
 */
function installHooksForTool(projectRoot, _tool, jonggrangInstallDir) {
  const results = {};
  ...
  const codexPath = installCodexHooks(projectRoot, jonggrangInstallDir);
  results.codex = { installed: true, path: codexPath };
