...
function isGitRepo(projectRoot) {
  try {
    require('child_process').execFileSync('git', ['rev-parse', '--is-inside-work-tree'], {
      cwd: projectRoot,
      stdio: 'ignore',
    });
    return true;
...
async function checkExitGates(projectRoot, fullOutput = '') {
  const ctx = { projectRoot, jonggrangLib: __dirname };
  const input = { tool_response: fullOutput, last_assistant_message: fullOutput };
  const gateHandlers = [handlers.feedbackLoop, handlers.qualityGate];
  if (isGitRepo(projectRoot)) {
    gateHandlers.push(handlers.outputEnforcement, handlers.secretFinalCheck);
  }
...
function createCodexRuntimeGuard({ projectRoot }) {
  return {
    ...
    checkExitGates(fullOutput = '') {
      return checkExitGates(projectRoot, fullOutput);
    },
    sanitizeText,
  };
}
