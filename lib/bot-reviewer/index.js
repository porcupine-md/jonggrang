...
function buildNote(review) {
  const emoji = { APPROVED: '✅', CHANGES_REQUESTED: '❌', COMMENT: '💬' }[review.verdict] ?? '💬';
  const issues = review.issues || [];

  const critical  = issues.filter(i => i.severity === 'CRITICAL' || i.severity === 'HIGH');
  const security  = issues.filter(i => i.type === 'security' && !critical.includes(i));
  const remaining = issues.filter(i => !critical.includes(i) && !security.includes(i));
