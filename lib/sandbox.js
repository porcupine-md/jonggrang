...
/**
 * Write a project file the way that project actually runs: inside the container
 * for a sandbox project, on the host otherwise.
 *
 * A sandbox container runs as root on a bind-mounted project, so every file it
 * touches ends up root-owned. The dashboard runs as the host user, so writing
 * those files from the host is a permission gamble it loses — saving a project's
 * Claude execution setting returned EACCES and the UI silently reverted. Writing
 * through the container is not a workaround; it is where a sandbox project's
 * files belong, and it keeps ownership consistent with everything else in there.
 *
 * The path is passed as an env var rather than interpolated into the shell, so a
 * project name with a space (or worse) cannot break or escape the command.
 *
 * Falls back to a host write when there is no container to write through
 * (stopped, or docker missing). That write is atomic — temp file plus rename —
 * which needs only the DIRECTORY to be writable, so it still succeeds against a
 * target the container previously took over.
 *
 * Returns 'container' or 'host', naming where the write landed.
 */
function writeProjectFile(project, relPath, content) {
    ...
    if (project?.sandbox?.enabled) {
        const dst = `${getContainerPath(project)}/${rel}`;
        try {
            execFileSync('docker', [
                'exec', '-i',
                '--env', `JG_DST=${dst}`,
                getContainerName(project.id),
                'sh', '-c', 'mkdir -p "$(dirname "$JG_DST")" && cat > "$JG_DST"',
            ], { input: body, maxBuffer: 64 * 1024 * 1024 });
            return 'container';
        } catch {
            // No container to write through — fall through to the host.
        }
    }
...
// Resolve where to READ a feature's live progress (.output/features/<fid>/).
// The isolated work-mode worktree is the source of truth while it exists (its
// worker writes tasks/manifest/progress there); once the worktree is gone we
// fall back to the main project's copy (the final snapshot taken at run end).
// Host-readable for both host and sandbox projects (sandbox worktrees are
// bind-mounted onto the host at projectWorktreeDir).
function featureOutputDir(project, featureId) {
    const wtOut = path.join(projectWorktreeDir(project.id), featureId, '.jonggrang', '.output', 'features', featureId);
    try { if (fs.statSync(wtOut).isDirectory()) return wtOut; } catch {}
    return path.join(project.path, '.jonggrang', '.output', 'features', featureId);
}
...
// True if the container's published editor port no longer matches the project's
// code_editor setting. The editor port is published at `docker run` only (Full
// mode), so toggling code_editor needs a recreate for the change to take effect.
// Returns false when there's no container to reconcile.
function editorMappingDrifted(project) {
    const { execFileSync } = require('child_process');
    let have;
    try {
        const out = execFileSync('docker', ['inspect', '--format',
            `{{if index .HostConfig.PortBindings "${EDITOR_CONTAINER_PORT}/tcp"}}1{{else}}0{{end}}`,
            getContainerName(project.id)], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim();
        have = out === '1';
    } catch {
        return false; // no container / inspect failed → nothing to reconcile
    }
    const want = project.code_editor === 'full';
    return want !== have;
}

module.exports = {
    getContainerName, getContainerPath, writeProjectFile,
    ...
    start, startExisting, stop, restart, remove, purgeProjectFiles, buildExecArgs,
