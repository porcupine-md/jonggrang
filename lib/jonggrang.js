...
function generateConfig(options) {
  ...
  return {
    ...
    skills: {
      directory: tool === 'opencode' ? './.opencode/skills' : './.claude/skills', // 'both' defaults to .claude
      custom: [],
    },
    review: {
      security: true,
...
function runInit(options, jonggrangHome, projectRoot) {
  ...
  if (fileExists(jonggrangSkillsDir)) {
    // Determine target skill directories based on selected tool
    const skillTargets = [];
    if (tool === 'claude' || tool === 'both') {
      skillTargets.push(path.join(projectRoot, '.claude', 'skills'));
    }
    if (tool === 'opencode' || tool === 'both') {
      skillTargets.push(path.join(projectRoot, '.opencode', 'skills'));
    }
    // Legacy fallback if no known tool
    if (skillTargets.length === 0) {
      skillTargets.push(path.join(projectRoot, 'skills'));
    }
