    ...
    if (tool === 'opencode') {
      ...
      (async () => {
        try {
          ...
          const { findExactModelReferenceMatch } = await import('@earendil-works/pi-coding-agent/dist/core/model-resolver.js');
          ...
          if (!atLineStart) process.stdout.write('\n');
          resolve(0);
          // Pi SDK keeps async handles alive (HTTP connections, timers) after prompt()
          // resolves. Force-exit so the subprocess does not hang and spawnSync returns.
          process.exit(0);
        } catch (err) {
...
function generateConfig(options) {
  ...
  return {
    ...
    design: {
      enabled: true,            // design phases auto-activate when triage classifies has_ui
      artifact: './DESIGN.md',  // canonical token spec, project root, git-tracked
  ...
  if (fileExists(jonggrangSkillsDir)) {
    ...
    try {
      ...
      for (const skillName of skillDirs) {
        ...
        const srcSkillDir = path.join(jonggrangSkillsDir, skillName);
        for (const targetBase of skillTargets) {
          const destSkillDir = path.join(targetBase, skillName);
          if (fileExists(path.join(destSkillDir, 'SKILL.md'))) continue;
          // Copy the whole skill directory — SKILL.md plus any references/ and
          // templates/ the skill ships (e.g. design-md's starter + research files).
