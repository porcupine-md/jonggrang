 ...
 */
function buildDraftPlanPrompt(description, configFile, tasksFile, srcPath = null) {
  let configSection = '';
 ...
 */
function buildDeepPlanDiscoveryPrompt(description, configFile, srcPath = null) {
  let configSection = '';
  if (configFile && fileExists(configFile)) {
    const cfg = readJSON(configFile);
    if (cfg) configSection = `## Project Config\n\`\`\`json\n${JSON.stringify(cfg, null, 2)}\n\`\`\`\n`;
 ...
 */
function buildDeepPlanCondensePrompt(description, discoveryContent, analysisContent, configFile, tasksFile, srcPath = null) {
  let completedSection = '';
  if (tasksFile && fileExists(tasksFile)) {
    const data = getTasks(tasksFile);
    const done = (data.tasks || []).filter(t => t.status === 'completed');
