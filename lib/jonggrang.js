...
const { createCodexRuntimeGuard } = require('./codex-runtime-guard');
...
function runAgent(prompt, tool, permMode, projectRoot, options = {}) {
  ...
  return new Promise((resolve) => {
    if (tool === 'opencode') {
      ...
      let buffer = '';
      let atLineStart = true;
      const codexOutputChunks = [];
      let runtimeGuardAborted = false;
      let childClosed = false;
      const runtimeGuard = createCodexRuntimeGuard({ projectRoot });

      function abortCodexRun(reason) {
        if (runtimeGuardAborted) return;
        runtimeGuardAborted = true;
        if (!atLineStart) process.stderr.write('\n');
      ...
      function handleCodexLine(line) {
        if (!line.trim()) return;
        debugLine(runtimeGuard.sanitizeText(line));
        let obj;
        try { obj = JSON.parse(line); } catch (err) {
          const redactedLine = runtimeGuard.sanitizeText(line);
          process.stderr.write(`[codex] JSON parse error: ${err.message}\n  line: ${redactedLine.slice(0, 200)}\n`);
          process.stdout.write(redactedLine + '\n');
          atLineStart = true;
          return;
        }

        const guardResult = runtimeGuard.inspectEvent(obj);
        if (guardResult.warning) process.stderr.write(`${guardResult.warning}\n`);
        if (guardResult.action === 'abort') abortCodexRun(guardResult.reason);
        ...
        if (type === 'item.started' && obj.item?.type === 'message') {
          ...
          if (item.type === 'agent_message' && typeof item.text === 'string') {
            const text = runtimeGuard.sanitizeText(item.text || '');
            process.stdout.write(text);
            atLineStart = text.endsWith('\n');
            if (text) codexOutputChunks.push(text);
            if (textChunks && text) textChunks.push(text);
          } else if (item.type === 'message' && item.role === 'assistant') {
            for (const c of (item.content || [])) {
              if (c.type === 'output_text') {
                const text = runtimeGuard.sanitizeText(c.text || '');
                process.stdout.write(text);
                atLineStart = text.endsWith('\n');
                if (text) codexOutputChunks.push(text);
                if (textChunks && text) textChunks.push(text);
            ...
            const args = (() => {
              try { return JSON.parse(item.arguments || '{}'); }
                ...
                const redactedArgs = runtimeGuard.sanitizeText(item.arguments || '');
                process.stderr.write(`[codex] function_call arguments parse error: ${err.message}\n  arguments: ${redactedArgs.slice(0, 200)}\n`);
                return {};
              }
            })();
      ...
      child.stderr.on('data', (d) => {
        ...
        if (filtered) process.stderr.write(runtimeGuard.sanitizeText(filtered));
      });

      child.on('close', async (code) => {
        childClosed = true;
        if (buffer.trim()) handleCodexLine(buffer);
        const gateResult = await runtimeGuard.checkExitGates(codexOutputChunks.join(''));
        if (gateResult.action === 'abort') abortCodexRun(gateResult.reason);
        if (!atLineStart) process.stdout.write('\n');
        resolve(finish(runtimeGuardAborted ? 1 : (code == null ? 1 : code)));
      });
