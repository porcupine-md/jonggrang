  ...
  return new Promise((resolve) => {
    if (tool === 'opencode') {
      ...
      (async () => {
        try {
          const os = require('os');
          const { createAgentSession, SessionManager, AuthStorage, ModelRegistry } = await import('@mariozechner/pi-coding-agent');

          // Reuse existing ~/.pi/agent if present, otherwise use ~/.jonggrang/agent
          const piAgentDir = path.join(os.homedir(), '.pi', 'agent');
          const jonggrangAgentDir = path.join(os.homedir(), '.jonggrang', 'agent');
          const agentDir = fs.existsSync(piAgentDir) ? piAgentDir : jonggrangAgentDir;
