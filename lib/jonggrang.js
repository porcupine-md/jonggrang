...
function migrateLegacyPlanDraft(projectRoot) {
  const paths = getProjectPaths(projectRoot);
  if (!fileExists(paths.planFile)) return null;
  let content = '';
  try { content = fs.readFileSync(paths.planFile, 'utf8'); } catch { return null; }
  if (!content.trim()) { try { fs.unlinkSync(paths.planFile); } catch {} return null; }
  ...
  fs.copyFileSync(paths.planFile, draftFileFor(projectRoot, sid));
  try { fs.unlinkSync(paths.planFile); } catch {}
  return sid;
}
...
function validateTasksFile(tasksFilePath) {
  // Relaxed for per-feature state: a missing root tasks file is VALID
  // (means "no feature yet" — tasks live under .output/features/<id>/).
  // Only corrupt/invalid-structure files are invalid. Used to detect
  // legacy root files that need migration.
  if (!fileExists(tasksFilePath)) return { valid: true, reason: 'no-root-file' };
  const data = readJSON(tasksFilePath);
  if (!data) return { valid: false, reason: 'corrupt' };
  if (!Array.isArray(data.tasks)) return { valid: false, reason: 'missing_field', field: 'tasks' };
  return { valid: true, reason: 'legacy-root' };
}

function validateProgressFile(progressFilePath) {
  // Relaxed for per-feature state: a missing root progress file is VALID
  // (progress lives under .output/features/<id>/progress.txt).
  if (!fileExists(progressFilePath)) return { valid: true, reason: 'no-root-file' };
  try {
...
function validateProjectState(projectRoot) {
  ...
  return { allValid: config.valid, config, tasks: { valid: true }, progress: { valid: true } };
}
...
// generateTaskId/addTask/addTasksBulk therefore scan ALL feature files via
// getAllTasks to continue numbering and detect collisions, even though they
// write to a single feature file. Read/update/remove operate on a single
// resolved feature file (tasksFile) — callers resolve the path first.

const VALID_STATUSES = new Set(['pending', 'in_progress', 'completed', 'blocked', 'waiting', 'skipped']);

function generateTaskId(projectRoot, featureId) {
  const all = getAllTasks(projectRoot);
  let maxNum = 0;
...
function addTask(projectRoot, featureId, taskData) {
  ...
  const data = getTasks(tasksFile);
  const id = taskData.id || generateTaskId(projectRoot, featureId);

  // Global uniqueness check across ALL features (not just this one)
  const all = getAllTasks(projectRoot);
...
function addTasksBulk(projectRoot, featureId, taskDataArray) {
  const tasksFile = tasksFileFor(projectRoot, featureId);
  const data = getTasks(tasksFile);
  // Global max across all features — assign IDs deterministically in bulk
  const all = getAllTasks(projectRoot);
  let maxNum = 0;
  ...
  for (const taskData of taskDataArray) {
    const id = taskData.id || (() => `task-${String(++maxNum).padStart(3, '0')}`)();
 ...
 */
function buildDeepPlanAnalysisPrompt(description, discoveryContent, analysisPath) {
