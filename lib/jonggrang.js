...
function runAgent(prompt, tool, permMode, projectRoot, options = {}) {
  ...
  return new Promise((resolve) => {
    if (tool === 'opencode') {
      ...
      // Spawned as: pi -p <prompt> [--model <model>] [--thinking <level>]
      const piArgs = ['-p', prompt, ...extraFlags];
      ...
      // Pi coding agent SDK — branded as Jonggrang
      // Pi is ESM-only, so dynamic import() is required from this CJS module.
      (async () => {
        try {
          const os = require('os');
          ...
          // Look up model saved by `jonggrang model` command
          ...
          const model = (cfgProvider && cfgModelId)
            ? modelRegistry.find(cfgProvider, cfgModelId)
            : undefined;
...
function runInit(options, jonggrangHome, projectRoot) {
  ...
  if (fileExists(jonggrangSkillsDir)) {
    // Always install skills for both tools + jonggrang (for Pi agent)
    const skillTargets = [
