...
const TUI_NOISE_RE = /(esc to interrupt|\? for shortcuts|ctrl\+[a-z]|bypass(ing)? permissions|shift\+tab|auto-accept edits)/i;
...
class TuiTranscript {
  ...
  // Escape sequences routinely straddle chunk boundaries, so ANSI is stripped
  // from whole lines only — an unterminated tail stays raw until it completes.
  push(chunk) {
    this.raw += String(chunk);
    const cut = Math.max(this.raw.lastIndexOf('\n'), this.raw.lastIndexOf('\r'));
    let ready;
    if (cut >= 0) {
  ...
  _emit(raw) {
    const line = normalizeTuiLine(raw);
    if (!line) return;
    if (this.recent.includes(line)) return;                   // same frame redrawn
...
function runClaudeInteractive({
  ...
  const args = [...permFlags, '--add-dir', projectRoot, ...extraFlags, prompt];
