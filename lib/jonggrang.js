 ...
 */
function buildDraftPlanPrompt(description, configFile, tasksFile, ingestedDoc = null) {
  let configSection = '';
...
${buildFeatureSection(description, ingestedDoc)}
 ...
 */
function buildDeepPlanDiscoveryPrompt(description, configFile, ingestedDoc = null) {
  let configSection = '';
  if (configFile && fileExists(configFile)) {
    const cfg = readJSON(configFile);
    if (cfg) configSection = `## Project Config\n\`\`\`json\n${JSON.stringify(cfg, null, 2)}\n\`\`\`\n`;
...
${buildFeatureSection(description, ingestedDoc, 'Feature Request')}
 ...
 */
function buildDeepPlanCondensePrompt(description, discoveryContent, analysisContent, configFile, tasksFile, ingestedDoc = null) {
  let completedSection = '';
  if (tasksFile && fileExists(tasksFile)) {
    const data = getTasks(tasksFile);
    const done = (data.tasks || []).filter(t => t.status === 'completed');
...
const INGEST_MAX_CHARS = 50000;

function truncateForIngest(text) {
  if (text.length <= INGEST_MAX_CHARS) return text;
  return text.slice(0, INGEST_MAX_CHARS) + '\n\n[... document truncated at 50,000 characters ...]';
}

/**
 * Build the feature section of a prompt, optionally injecting an ingested document.
 * @param {string} description - user-provided description (may be empty if file given)
 * @param {object|null} ingestedDoc - result of ingestDocument(), or null
 * @param {string} heading - section heading (default: 'Feature Description')
 */
function buildFeatureSection(description, ingestedDoc, heading = 'Feature Description') {
  if (!ingestedDoc) return `## ${heading}\n${description}`;
  const descLine = description
    ? `## ${heading}\n${description}`
    : `## ${heading}\n(from document: ${ingestedDoc.filename})`;
  return `${descLine}\n\n## Requirements Document (${ingestedDoc.filename})\n${ingestedDoc.text}`;
}

/**
 * Read a BRD/PRD file and extract its text content.
 * Supports .md, .txt (direct read) and .pdf (via pdf-parse).
...
async function ingestDocument(filePath) {
  ...
  if (ext === '.md' || ext === '.txt') {
    return { text: truncateForIngest(fs.readFileSync(filePath, 'utf8')), filename, format: ext.slice(1) };
  }

  if (ext === '.pdf') {
    // Lazy require: pdf-parse is only needed for the .pdf branch, so plain-text
    // plans don't pay the parse-library load cost.
    const pdfParse = require('pdf-parse');
    const data = await pdfParse(fs.readFileSync(filePath));
    return { text: truncateForIngest(data.text), filename, format: 'pdf' };
  }

  throw new Error(`Unsupported file format: ${ext || '(none)'}. Supported: .md, .txt, .pdf`);
}
