...
const uiContext = require('./ui-context');
...
// The feature a run is currently executing, recorded in the project itself
// rather than in the environment. A worktree run knows its plan; the agent's own
// `jonggrang task ...` calls are separate processes, and on a device project
// they run over ssh on the developer's machine — where no env var from the
// server survives. A file in the worktree travels with the worktree, so every
// context (host, container, device) resolves the same answer.
//
// Per-feature task numbering means `task-001` exists in every plan. Without this
// the resolver falls back to guessing the "active" feature, which in a worktree
// running one plan while another is live silently writes to the wrong plan.
function scopedFeatureFile(projectRoot) {
  return path.join(projectRoot, '.jonggrang', '.ephemeral', 'active-feature');
}

function writeScopedFeature(projectRoot, featureId) {
  if (!featureId) return false;
  try {
    const file = scopedFeatureFile(projectRoot);
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, `${featureId}\n`);
    return true;
  } catch {
    // Advisory only — a run must not fail because a hint could not be written.
    return false;
  }
}

function readScopedFeature(projectRoot) {
  if (process.env.JONGGRANG_FEATURE_ID) return process.env.JONGGRANG_FEATURE_ID;
  try { return fs.readFileSync(scopedFeatureFile(projectRoot), 'utf8').trim() || null; }
  catch { return null; }
}
...
// disambiguate in order: (1) explicit opts.featureId, (2) the feature the
// current run recorded (readScopedFeature), (3) the active feature, (4) a single
// global match (legacy globally-unique ids keep working). If still
...
function findTaskFeature(projectRoot, taskId, opts = {}) {
  ...
  const scoped = readScopedFeature(projectRoot);
  if (scoped && matches.some(t => t.feature_id === scoped)) return scoped;
...
function makeTask(taskData, id, featureId, defaultPriority) {
  return {
    ...
    completed_at: null,
    error_log: [],
    ...(taskData.ui_context ? { ui_context: taskData.ui_context } : {}),
  };
}
...
function buildWorkPrompt(taskId, tasksFile, mode, testFeedback) {
  ...
  const uiSection = uiContext.buildTaskUiPrompt(uiContext.projectRootFromStateFile(tasksFile), task);
...
function buildDraftPlanPrompt(description, configFile, projectRoot, draftPath, srcPath = null, opts = {}) {
  ...
  const clarSection = opts.clarifications
    ...
    : '';
  const uiSection = opts.ui ? `${opts.ui.prompt}\n\n` : '';
  const uiFrontmatter = opts.ui
    ? `\nui: true\nui_guide_status: ${opts.ui.guideStatus}\nui_baseline: ${opts.ui.baseline || 'ask-user'}\nui_token_status: ready|planned`
    : '';
...
${completedSection}${clarSection}${uiSection}## Your Task
...
- Do NOT write to the tasks file — tasks come in Phase 2 after human review${opts.ui ? '\n- Complete the UI planning sidecars named in the UI Planning Contract before reporting success.' : ''}
...
- 3-8 phases max, high-level; do NOT write code or the tasks file.${deepRule}${opts.ui ? '\n- Complete the UI planning sidecars named in the UI Planning Contract before reporting success.' : ''}
- After writing the plan, output exactly: "Draft plan written to ${draftPath}"`;
}
...
function buildRevisePlanPrompt(currentPlanContent, feedback, draftFile, opts = {}) {
  const clarSection = opts.clarifications
    ...
    : '';
  const uiSection = opts.ui
    ? `\n${opts.ui.prompt}\n\nRevise the UI sidecars together with the plan when the feedback changes a UI decision. Preserve them when it does not.\n`
    : '';
...
- Preserve UI frontmatter and sidecars unless the feedback explicitly changes UI scope or decisions
 ...
 */
function buildTasksFromPlanPrompt(planContent, configFile, projectRoot, featureId, skillsDir, opts = {}) {
  let skillsList = '';
...
Approved guide revision: \`${ui.guideRevision}\`
Approved baseline: \`${ui.baseline}\`
Final handoff path: \`${ui.handoffPath}\`
Token source: \`${ui.tokenSource}\` (${ui.tokenStatus})
${ui.tokenStatus === 'planned' && ui.baselineTokenTemplate ? `
Approved starter token template — the UI-foundation task must copy this exact
content to \`${ui.tokenSource}\` and include it verbatim in its handoff task
section so approval can verify that the pack reached runtime:

\`\`\`css
${ui.baselineTokenTemplate.trim()}
\`\`\`
` : ''}
Approved project guide:
\`\`\`markdown
${ui.guideContent}
\`\`\`

Approved feature direction draft:
\`\`\`markdown
${ui.handoffDraftContent}
\`\`\`

For every task that changes visible UI, component behavior, styles, responsive
behavior, interaction state, or accessibility, include \`ui_context\`. Do not add
it to backend-only tasks. Use this exact shape:

\`\`\`json
{
  "handoff": "${ui.handoffPath}",
  "sections": ["Feature intent", "Shared direction", "Task task-001"],
  "guide": ".jonggrang/UI.md",
  "guide_revision": "${ui.guideRevision}",
  ...
  "token_source": "${ui.tokenSource}",
  "source_files": ["real/project/path"],
  "states": ["loading", "empty", "error", "ready"],
  "verification": ["real project command"]
}
\`\`\`

Replace \`task-001\` with that task's actual id. Select only relevant guide
sections and real source files. If the token source is planned, create exactly
one first UI-foundation task with \`ui_context.foundation: true\`; every later UI
task must depend on it. The foundation task creates the approved token source
and changes the guide's owned token fields from planned to ready.${ui.tokenStatus === 'planned' && ui.baselineTokenTemplate ? ' Its handoff section must contain the approved starter token template verbatim; do not paraphrase or regenerate token values.' : ''}

After importing tasks, write the complete handoff to \`${ui.handoffAbsolutePath}\`.
${ui.append ? 'This is APPEND MODE: preserve every existing heading and task section byte-for-byte, then extend shared direction/references only when the approved extension requires it and append sections for NEW UI task ids. Never remove or rewrite an earlier task contract.\n' : ''}Keep the approved Feature intent, Shared direction, and References, then add one
\`## Task task-xxx\` section per UI task. Each section must include Objective,
Use, Change, States, Do not, Acceptance, Sources, and Check. Do not add sections
...
\`\`\`json
{
  ...
  "blocked_by": ["task-001"]${ui ? ',\n  "ui_context": { "...": "required for UI tasks; use the approved shape above" }' : ''}
}
\`\`\`
...
- Cover ALL phases — do not skip any phase from the plan${ui ? '\n- UI tasks must carry bounded ui_context and have matching handoff sections. Backend-only tasks must not carry ui_context.\n- Write and validate the final UI_HANDOFF.md after task import before reporting success.' : ''}
...
function buildReviewPrompt(projectRoot = process.cwd(), featureId = null) {
  let uiSection = '';
  if (featureId) {
    const handoff = uiContext.featureHandoffPath(projectRoot, featureId);
    if (fileExists(handoff)) {
      const content = fs.readFileSync(handoff, 'utf8');
      const selected = ['Feature intent', 'Shared direction', 'References']
        .map(section => uiContext.extractMarkdownSection(content, section))
        .filter(Boolean)
        .join('\n\n');
      uiSection = `\n## Feature UI Review Context\n\n${selected}\n\nRead \`.jonggrang/UI.md\` only for rules referenced by the handoff. Report \`UI_GUIDE_DRIFT\` rather than inventing a reconciliation.\n`;
    }
  }
  return `# Jonggrang Review Session

${buildProjectContext(projectRoot, { maxChars: 3000 })}
...
### UI (when a feature handoff is present)
- Matches feature intent and required states
- Reuses named components and semantic tokens
- Keyboard, focus, responsive, loading, empty, and error behavior are verified
- Any guide/code conflict is reported as UI_GUIDE_DRIFT
...
// ============================================================
//
// Opt-in alternative to headless `claude -p`: runs the real Claude Code TUI
// inside a pty, so the session behaves like a human-driven one (background
// tasks, parallel subagents, anything the print mode restricts).
//
// Selected per project with `tools.claude.execution: "interactive"` in
// .jonggrang/jonggrang.json (JONGGRANG_CLAUDE_EXEC overrides it for debugging).
// The default stays "headless" — existing projects keep today's behavior.
//
// Sandbox works unchanged: the pty is created by the node process INSIDE the
// container, so `docker exec -i` (no -t) is still enough.

const CLAUDE_IDLE_DEFAULT_SEC = 15;   // a quiet pty = the turn is over (the TUI animates while busy)
const CLAUDE_MIN_RUNTIME_MS   = 5000; // never call it idle while the TUI is still booting
// Linux caps a single argv entry at 128 KiB (MAX_ARG_STRLEN) — stay well under.
const CLAUDE_ARGV_PROMPT_MAX  = 96 * 1024;

// Live ptys, so a SIGTERM (web "Cancel") never leaves an orphan claude holding
// the worktree. Wired lazily — the CLI's signal behavior is untouched until an
// interactive session actually starts.
const _livePtys = new Set();
let _ptySignalsWired = false;

function wirePtySignals() {
  if (_ptySignalsWired) return;
  _ptySignalsWired = true;
  const killAll = () => {
    for (const p of _livePtys) { try { p.kill(); } catch { /* already gone */ } }
    _livePtys.clear();
  };
...
function claudePermissionFlags(permMode) {
  switch (permMode) {
    case 'autonomous': return ['--dangerously-skip-permissions'];
    case 'balanced':   return ['--permission-mode', 'acceptEdits'];
    case 'supervised': return ['--permission-mode', 'default'];
    default:           return [];
  }
}

function claudeConfigPath(projectRoot) {
  return path.join(projectRoot || process.cwd(), '.jonggrang', 'jonggrang.json');
}

/** "interactive" | "headless" — env override wins, then project config, then default. */
function resolveClaudeExecution(projectRoot) {
  const env = String(process.env.JONGGRANG_CLAUDE_EXEC || '').trim().toLowerCase();
  if (env === 'interactive' || env === 'headless') return env;
  const cfg = readConfig(claudeConfigPath(projectRoot), 'tools.claude.execution', 'headless');
  return String(cfg).toLowerCase() === 'interactive' ? 'interactive' : 'headless';
}

function claudeNumberSetting(projectRoot, key, fallback) {
  const raw = readConfig(claudeConfigPath(projectRoot), `tools.claude.${key}`, '');
...
/**
 * Decide whether THIS run can use the interactive path. Any reason it cannot is
 * reported once and the caller silently falls back to the headless branch —
 * an unusable pty must degrade, never fail the run.
 */
function useClaudeInteractive(prompt, projectRoot) {
  if (resolveClaudeExecution(projectRoot) !== 'interactive') return false;
  try {
    require('node-pty');
  } catch (err) {
    process.stderr.write(`\x1b[33m[claude:interactive] node-pty unavailable (${err.message}) — using headless mode\x1b[0m\n`);
    return false;
  }
  if (Buffer.byteLength(prompt, 'utf8') > CLAUDE_ARGV_PROMPT_MAX) {
    process.stderr.write(`\x1b[33m[claude:interactive] prompt too large for argv (>${Math.round(CLAUDE_ARGV_PROMPT_MAX / 1024)}KB) — using headless mode\x1b[0m\n`);
    return false;
  }
  return true;
}
...
// A TUI redraws whole frames; the web log panel is append-only. These filters
// turn frames back into a readable line transcript.
const TUI_BOX_RE   = /^[\s─━│┃┌┐└┘├┤┬┴┼╭╮╯╰═║╔╗╚╝▔▁▏▕·•⋯…>_]*$/;
const TUI_NOISE_RE = /(esc to interrupt|for shortcuts|for agents|ctrl\+[a-z]|bypass(ing)? permissions|shift\+tab|auto-accept edits|^❯?\s*Try ")/i;

// Claude Code asks "Is this a project you created or one you trust?" the first
// time it opens a directory — including every fresh worktree. In a TUI that
// dialog is indistinguishable from a finished turn, so the run would idle out
// having done nothing. Answered automatically ONLY in autonomous mode, where
// --dangerously-skip-permissions has already granted more than this, and always
// logged. Any other mode leaves the question to the human.
const TUI_TRUST_RE = /(Yes, I trust this folder|Quick safety check|Do you trust the files in this folder)/i;

// Claude Code lays out a line by jumping the cursor to absolute columns rather
// than emitting spaces — a word gap is `ESC[12G`, not "   ". Stripping escapes
// alone therefore glues every word together ("ClaudeCodev2.1.233"), so one
// physical line is replayed onto a column buffer instead: cursor moves become
// the padding they represent, and everything else (colour, erase) is dropped.
function renderTuiLine(raw) {
  const s = String(raw);
  const rows = [];
  let out = [];
  let col = 0;

  // A vertical cursor move means the TUI left this screen row — end the rendered
  // line there so the next frame cannot overwrite unrelated words on it.
  const breakRow = () => { rows.push(out.join('')); out = []; col = 0; };

  for (let i = 0; i < s.length;) {
    const ch = s[i];

    if (ch === '\u001B') {
      const csi = /^\u001B\[([0-9;?]*)[ -\/]*([@-~])/.exec(s.slice(i));
      if (csi) {
        const [, params, final] = csi;
        const n = parseInt(params, 10);
        const count = Number.isFinite(n) ? n : 1;
        if (final === 'G') col = Math.max(0, count - 1);                       // column absolute
        else if (final === 'C') col += count;                                  // cursor forward
        else if (final === 'D') col = Math.max(0, col - count);                // cursor back
        else if (final === 'H' || final === 'f') {                             // row;col
          const c = parseInt(params.split(';')[1], 10);
          breakRow();
          col = Math.max(0, (Number.isFinite(c) ? c : 1) - 1);
        } else if (final === 'A' || final === 'B' || final === 'E' || final === 'F') {
          breakRow();                                                          // moved off this row
        } else if (final === 'K') {                                            // erase in line
          if (params === '' || params === '0') out.length = Math.min(out.length, col);
          else if (params === '1') for (let c2 = 0; c2 < col && c2 < out.length; c2++) out[c2] = ' ';
          else if (params === '2') out.length = 0;
        }
        i += csi[0].length;
        continue;
      }
      const osc = /^\u001B\][^\u0007\u001B]*(?:\u0007|\u001B\\)/.exec(s.slice(i));
      if (osc) { i += osc[0].length; continue; }
      const fe = /^\u001B[@-Z\\-_]/.exec(s.slice(i));
      i += fe ? fe[0].length : 1;
      continue;
    }
...
// Animated status glyphs. `●` is deliberately NOT here: Claude Code prefixes
// its tool calls and answers with it, which is the content worth keeping.
const TUI_SPINNER = '✢✳✶✻✽◐◓◑◒○◌';

function normalizeTuiLine(raw) {
  const s = String(raw).replace(/[│┃║]/g, ' ').replace(/\s+$/, '');
  const t = s.trim();
  if (!t) return '';
  if (TUI_BOX_RE.test(t)) return '';
  if (TUI_NOISE_RE.test(t)) return '';
  // A lone glyph or caret is one frame of an animation, not a line of output.
  if (new RegExp(`^[${TUI_SPINNER}●❯>*·•\\s]{1,3}$`).test(t)) return '';
  if (new RegExp(`^[${TUI_SPINNER}]`).test(t)) return '';          // "✻ Crunched for 1s"
  // Spinner frames ("✻ Thinking… (12s · ↑ 1.2k tokens)") change every 100ms.
  if (/\(\s*\d+m?\s*\d*s\b/.test(t) && /[·•]/.test(t)) return '';
  // Status word mid-animation ("Jitterbugging…", "Flummoxin   …").
  if (/…$/.test(t) && t.length <= 40 && !/[:/]/.test(t)) return '';
  // Terminal capability replies the emulator echoes back (DA/DSR/XTVERSION).
  if (/^\[?[>?][0-9;]*[a-zA-Z]$/.test(t)) return '';
  if (/^\d{1,3}$/.test(t)) return '';
  return s;
}

/** Frame-to-transcript converter: strips ANSI/box chrome and drops repeated frames. */
class TuiTranscript {
  constructor({ onLine, historySize = 120 }) {
    this.onLine = onLine;
  ...
  // Escape sequences routinely straddle chunk boundaries, so a line is only
  // rendered once its newline arrives — an unterminated tail stays raw. CR is
  // NOT a break: the TUI uses it to rewrite the line it is already on.
  push(chunk) {
    this.raw += String(chunk);
    const cut = this.raw.lastIndexOf('\n');
    let ready;
    if (cut >= 0) {
      ready = this.raw.slice(0, cut + 1);
      this.raw = this.raw.slice(cut + 1);
    } else if (this.raw.length > 8192) {
      ready = this.raw;
      this.raw = '';
    } else {
      return;
  ...
  flush() {
    if (this.raw) this._consume(this.raw);
    this.raw = '';
  }
  ...
  _emit(rawLine) {
    // Replay the line's cursor moves onto a column buffer first, THEN filter —
    // the spacing only exists as escapes (see renderTuiLine). One physical line
    // can render to several rows when the cursor jumps vertically.
    for (const rendered of renderTuiLine(rawLine).split('\n')) {
      const line = normalizeTuiLine(rendered);
      if (!line) continue;
      if (this.recent.includes(line)) continue;               // same frame redrawn
      this.recent.push(line);
      if (this.recent.length > this.historySize) this.recent.shift();
      this.onLine(line.length > 2000 ? `${line.slice(0, 2000)}…` : line);
    }
  }
}

// ── Session transcript (the accurate log source) ──────────────
//
// Reconstructing a log by scraping the TUI is lossy by nature: Claude Code
// paints a SCREEN — wrapping, rewriting and scrolling — so a line-oriented
// reader drops characters no matter how carefully it tracks columns. The same
// session is also written to disk as structured JSONL, one record per event,
// which cannot be garbled. That file is the log source; the screen scrape stays
// only as a fallback for when it cannot be found.

const CLAUDE_SESSION_POLL_MS     = 300;
const CLAUDE_SESSION_DISCOVER_MS = 20000;   // no session file by now → fall back

// ── Live session mirroring ────────────────────────────────────
//
// With JONGGRANG_PTY_SESSION set, the worker mirrors the raw pty to the
// dashboard and accepts keystrokes back, so the agent's TUI can be watched and
// typed into exactly like the design studio's — while jonggrang keeps driving
// the routine input. Frames are line-delimited JSON on stdout/stdin, the same
// channel task_status signals already travel on; the payload is base64 so ANSI
// bytes survive JSON and can never be mistaken for a log line.
const PTY_STREAM_FLUSH_MS = 50;             // coalesce redraws instead of one frame per chunk
const PTY_STREAM_MAX_CHUNK = 64 * 1024;

/** Buffers raw pty output and ships it as `pty_data` frames. */
function createPtyMirror(session) {
  let pending = [];
  let bytes = 0;
  let timer = null;
  ...
  function flush() {
    ...
    const payload = Buffer.concat(pending).toString('base64');
    pending = [];
    bytes = 0;
    process.stdout.write(`${JSON.stringify({ type: 'pty_data', session, b64: payload })}\n`);
  }

  return {
    write(data) {
      pending.push(Buffer.from(String(data), 'utf8'));
      bytes += data.length;
      if (bytes >= PTY_STREAM_MAX_CHUNK) { clearTimeout(timer); flush(); return; }
      if (!timer) timer = setTimeout(flush, PTY_STREAM_FLUSH_MS);
    },
    close(code) {
      clearTimeout(timer);
      flush();
      process.stdout.write(`${JSON.stringify({ type: 'pty_exit', session, code })}\n`);
    },
  };
}

/** Applies `pty_input` / `pty_resize` frames from the server onto the pty. */
function attachPtyInput(child) {
  let buf = '';
  const onData = (chunk) => {
    buf += chunk.toString();
    let idx;
    while ((idx = buf.indexOf('\n')) >= 0) {
      ...
      try {
        if (msg.type === 'pty_input' && msg.b64) child.write(Buffer.from(msg.b64, 'base64').toString('utf8'));
        else if (msg.type === 'pty_resize' && msg.cols > 0 && msg.rows > 0) child.resize(msg.cols, msg.rows);
...
/** ~/.claude/projects/<cwd with / and . replaced by ->  (CLAUDE_CONFIG_DIR aware). */
function claudeSessionDir(projectRoot) {
  const base = process.env.CLAUDE_CONFIG_DIR || path.join(require('os').homedir(), '.claude');
  return path.join(base, 'projects', String(projectRoot).replace(/[/.]/g, '-'));
}
...
/**
 * Follow the JSONL transcript this run's session is writing and emit the same
 * shape the headless stream-json path prints: assistant text, one line per tool
 * call, and tool errors. Thinking blocks are skipped, as they are headless.
 */
class ClaudeSessionTail {
  constructor({ projectRoot, since, onText, onLine }) {
    this.dir = claudeSessionDir(projectRoot);
    this.since = since;
    this.onText = onText;
    this.onLine = onLine;
    this.file = null;
  ...
  /** True once a session file for THIS run has been found. */
  get active() { return this.file !== null; }

  // Newest .jsonl touched after this run started. Concurrent sessions in one
  // directory would be ambiguous; parallel plans each get their own worktree
  // (and therefore their own directory), so in practice there is exactly one.
  _discover() {
    let newest = null;
    let newestMtime = 0;
    let entries;
    ...
    for (const name of entries) {
      ...
      if (stat.mtimeMs + 1000 < this.since) continue;              // predates this run
      if (stat.mtimeMs > newestMtime) { newestMtime = stat.mtimeMs; newest = full; }
    }
    if (newest) { this.file = newest; this.offset = 0; }
  }

  poll() {
    if (!this.file) this._discover();
    if (!this.file) return;
    ...
    for (const line of lines) this._record(line);
  }
  ...
  _record(line) {
    ...
    if (obj.uuid && this.seenIds.has(obj.uuid)) return;
    if (obj.uuid) this.seenIds.add(obj.uuid);
    ...
    for (const block of content) {
      if (block.type === 'text' && block.text) {
        this.onText(block.text.endsWith('\n') ? block.text : `${block.text}\n`);
      } else if (block.type === 'tool_use') {
        const detail = String(toolDetail(block.input)).split('\n')[0];
        const short = detail.length > 100 ? `${detail.slice(0, 97)}...` : detail;
        this.onLine(`\x1b[90m▸ ${block.name || '?'}\x1b[0m${short ? ` \x1b[2m${short}\x1b[0m` : ''}`);
      } else if (block.type === 'tool_result' && block.is_error) {
        const msg = String(block.content ?? '').split('\n')[0].slice(0, 120);
        this.onLine(`  \x1b[31m✗ ${msg}\x1b[0m`);
      }
    }
  }
}
...
/**
 * Run one claude turn through the TUI and resolve with an exit code.
 *
 * Completion is detected by idleness: Claude Code animates its status line the
 * whole time it is working, so a pty that has produced nothing for
 * `tools.claude.idle_timeout_sec` is a session waiting for input — i.e. the turn
 * is done. The session is then closed with /exit, escalating to ^C and finally
 * kill(). `runIteration` still verifies the task was actually marked complete,
 * so a wrong guess costs a retry, not a corrupt run.
 */
function runClaudeInteractive({
  prompt, permFlags = [], extraFlags = [], projectRoot, textChunks, debug,
  // Overrides — used by the tests; production reads them from the project config.
  idleSec: idleSecOpt, maxSec: maxSecOpt, exitGraceMs = 5000, killGraceMs = 3000, minRuntimeMs = CLAUDE_MIN_RUNTIME_MS,
}) {
  const ptyLib = require('node-pty');
  const idleSec = idleSecOpt != null ? idleSecOpt : claudeNumberSetting(projectRoot, 'idle_timeout_sec', CLAUDE_IDLE_DEFAULT_SEC);
  const maxSec  = maxSecOpt  != null ? maxSecOpt  : claudeNumberSetting(projectRoot, 'max_runtime_sec', 0);
  // NO --add-dir here, unlike the headless path. Its parameter is variadic
  // (`--add-dir <directories...>`), so a trailing prompt is swallowed as a second
  // directory and the TUI opens with an empty input box — the session then sits
  // idle and the run reports success having done nothing. The headless path is
  // immune because it writes the prompt to stdin. cwd already is projectRoot.
  const args = [...permFlags, ...extraFlags, prompt];

  return new Promise((resolve) => {
    const startedAt = Date.now();
    let settled = false, closing = false, sawOutput = false, closeCode = 0;
    let idleTimer = null, maxTimer = null, exitTimer = null;

    const autoTrust = permFlags.includes('--dangerously-skip-permissions');
    ...
    // The JSONL transcript is the log; the screen scrape is watched regardless
    // because the folder-trust dialog only exists on screen — it never reaches
    // the session file — but its lines are printed only while JSONL is absent.
    const session = new ClaudeSessionTail({
      projectRoot,
      since: Date.now(),
      onText: (t) => emit(t),
      onLine: (l) => emit(`${l}\n`),
    ...
    // Screen lines are held back until we know whether a session transcript
    // exists: printing them eagerly would put the splash screen — and every
    // lossy redraw — in the log before the accurate source is discovered.
    ...
    process.stdout.write(`\x1b[2m[claude:interactive] TUI session — idle cutoff ${idleSec}s${maxSec ? `, max ${maxSec}s` : ''}\x1b[0m\n`);

    let child;
    try {
      child = ptyLib.spawn('claude', args, {
        name: 'xterm-256color',
        cols: 120,
        rows: 40,
        cwd: projectRoot,
      ...
      process.stderr.write(`\x1b[31m[claude:interactive] spawn failed: ${err.message}\x1b[0m\n`);
      return resolve(1);
    }
    ...
    // Mirror the session to the dashboard when the server asked for it.
    const mirrorSession = process.env.JONGGRANG_PTY_SESSION || '';
    const mirror = mirrorSession ? createPtyMirror(mirrorSession) : null;
    const detachInput = mirror ? attachPtyInput(child) : null;
    if (mirror) {
      process.stdout.write(`\x1b[2m[claude:interactive] live session mirrored to the dashboard (${mirrorSession})\x1b[0m\n`);
    }

    // Follow the session file for the duration of the run.
    let announcedSource = false;
    const sessionTimer = setInterval(() => {
      session.poll();
      if (session.active && !announcedSource) {
        announcedSource = true;
        pendingScreen.length = 0;                                 // the screen was only a stand-in
        process.stdout.write('\x1b[2m[claude:interactive] streaming the session transcript\x1b[0m\n');
      }
      if (!session.active && !announcedSource && Date.now() - startedAt > CLAUDE_SESSION_DISCOVER_MS) {
        announcedSource = true;
        screenFallback = true;
        process.stdout.write('\x1b[33m[claude:interactive] no session transcript found — falling back to screen capture (log may be lossy)\x1b[0m\n');
        for (const line of pendingScreen.splice(0)) emit(`${line}\n`);
      }
    ...
    function done(code) {
      if (settled) return;
      settled = true;
      clearTimeout(idleTimer); clearTimeout(maxTimer); clearTimeout(exitTimer);
      clearInterval(sessionTimer);
      _livePtys.delete(child);
      if (detachInput) detachInput();
      if (mirror) mirror.close(code);
      session.poll();                                             // drain the tail
      transcript.flush();
      // Short run that ended before a transcript appeared: the held-back screen
      // lines are all the log there is, so print them rather than lose them.
      if (!session.active && pendingScreen.length) {
        for (const line of pendingScreen.splice(0)) emit(`${line}\n`);
      }
      resolve(code);
    }

    // Ask the TUI to quit, then escalate. `code` is what the run reports once
    // the session is actually gone.
    function closeSession(reason, code) {
      if (closing || settled) return;
      closing = true;
      closeCode = code;
      clearTimeout(idleTimer); clearTimeout(maxTimer);
      process.stdout.write(`\x1b[2m[claude:interactive] ${reason} — closing session\x1b[0m\n`);
      try { child.write('/exit\r'); } catch { /* pty already gone */ }
      exitTimer = setTimeout(() => {
        try { child.write('\x03'); child.write('\x03'); } catch { /* ignore */ }
        exitTimer = setTimeout(() => { try { child.kill(); } catch { /* ignore */ } done(code); }, killGraceMs);
      }, exitGraceMs);
    }
    ...
    function armIdle() {
      ...
      idleTimer = setTimeout(() => {
        if (!sawOutput || Date.now() - startedAt < minRuntimeMs) { armIdle(); return; }
        closeSession(`idle for ${idleSec}s`, 0);
      }, idleSec * 1000);
    }

    if (maxSec > 0) maxTimer = setTimeout(() => closeSession(`max runtime ${maxSec}s reached`, 1), maxSec * 1000);
    armIdle();

    child.onData((data) => {
      sawOutput = true;
      if (mirror) mirror.write(data);                             // live view gets the raw bytes
      if (debug) process.stderr.write(`\x1b[36m\x1b[2m[pty]\x1b[0m ${JSON.stringify(String(data).slice(0, 400))}\n`);
    ...
    child.onExit(({ exitCode }) => {
      done(closing ? closeCode : (exitCode || 0));
    });
  });
}
...
function runAgent(prompt, tool, permMode, projectRoot, options = {}) {
  ...
  return new Promise((resolve) => {
    if (tool === 'opencode') {
      ...
      const claudeFlags = claudePermissionFlags(permMode);

      // Opt-in interactive (PTY) execution. Everything else about the run —
      // prompt, cwd, model/effort flags — is identical to the headless path.
      if (useClaudeInteractive(prompt, projectRoot)) {
        runClaudeInteractive({ prompt, permFlags: claudeFlags, extraFlags, projectRoot, textChunks, debug })
          .then((code) => resolve(finish(code)));
        return;
      }
      ...
      // Device projects: the agent runs here and its Bash on the developer's
      // machine. Without being told, it writes commands for THIS platform — GNU
      // `sed -i` against a BSD userland — and they fail for reasons it cannot
      // diagnose. The orchestrator passes the sentence to say.
      if (process.env.JONGGRANG_DEVICE_PROMPT) {
        args.push('--append-system-prompt', process.env.JONGGRANG_DEVICE_PROMPT);
      }
      // The Bash-redirect hook lives in a server-side bundle of its own, so it
      // survives `init` rewriting the project's own .claude/settings.json.
      if (process.env.JONGGRANG_DEVICE_SETTINGS) {
        args.push('--settings', process.env.JONGGRANG_DEVICE_SETTINGS);
      }
...
const BASE_STATE_PATHS = [
  '.jonggrang/.output/features/*/plan.md',
  '.jonggrang/.output/features/*/UI_HANDOFF.md',
  '.jonggrang/UI.md',
  '.jonggrang/codemap/codemap.json',
];
...
6. **Test patterns** — How are tests currently structured? What test helpers exist?${opts.ui ? '\n7. **UI evidence** — Confirm token/theme sources, shared components, representative screens, states, accessibility, and visual verification named by the UI audit.' : ''}
 ...
 */
function buildDeepPlanAnalysisPrompt(description, discoveryContent, analysisPath, srcPath = null, opts = {}) {
  const uiSection = opts.ui ? `\n${opts.ui.prompt}\n` : '';
...
- Do NOT write to the tasks file${opts.ui ? '\n- Complete the UI planning sidecars named in the UI Planning Contract before reporting success.' : ''}
 ...
 */
function buildPlanQuestionsPrompt(description, configFile, srcPath = null, baseBranch = null, opts = {}) {
...
${configSection}${buildBaseBranchContext(branchToUse)}${uiSection}
...
2. **Decide if you can plan confidently.** If anything MATERIAL is ambiguous —
   ...
   UX, baseline selection when the product shape is uncertain, or a decision
   with real trade-offs — DO NOT guess.
...
Rules:
- Ask ONLY what genuinely blocks a correct plan. At most ${MAX_PLAN_QUESTIONS} questions.${opts.ui && opts.ui.requiresBaselineConsent ? '\n- The platform will prepend the mandatory `ui-preference` question. Do not duplicate baseline consent; ask only other material UI uncertainties.' : ''}
