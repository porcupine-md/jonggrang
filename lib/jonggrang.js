...
6. If all pass, commit with message format: "type(scope): description". Always add a trailing blank line then "${COAUTHOR_TRAILER}" as the last line of the commit message.
    ...
    if (tool === 'opencode') {
      ...
      (async () => {
        try {
          ...
          // resolves. Use setImmediate so the microtask queue (resumed awaits in the
          // caller, e.g. cmdApprove's archive step) runs before the process exits.
