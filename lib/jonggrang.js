...
/**
 * Extract a plan block from raw agent text output.
 * Returns the plan content string if found (from first `---` frontmatter to end),
 * or null if no plan-like structure is detected.
 */
function extractPlanFromText(text) {
  if (!text) return null;
  // Look for YAML frontmatter opening with a plan-specific key
  const fmMatch = text.match(/(---\s*\nfeature:\s*[\s\S]+)/);
  if (fmMatch) return fmMatch[1].trim();
  // Fallback: look for "# Plan:" markdown heading
  const headingMatch = text.match(/(# Plan:[\s\S]+)/);
...
function runAgent(prompt, tool, permMode, projectRoot, options = {}) {
  ...
  return new Promise((resolve) => {
    const done = (code) => resolve(captureText ? { code, text: capturedLines.join('') } : code);
      ...
      function handleOpenCodeLine(line) {
        ...
        if (obj.type === 'text') {
          ...
          if (text) {
            ...
            if (captureText) capturedLines.push(text);
          }
        ...
        if (obj.type === 'stream_event') {
          ...
          if (ev.type === 'message_start') {
            ...
            if (delta.type === 'text_delta' && !inToolBlock) {
              ...
              if (captureText) capturedLines.push(delta.text);
