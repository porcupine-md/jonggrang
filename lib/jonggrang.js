 ...
 */
function buildDraftPlanPrompt(description, configFile, projectRoot, draftPath, srcPath = null) {
  let configSection = '';
 ...
 */
function buildDeepPlanDiscoveryPrompt(description, configFile, discoveryPath, srcPath = null) {
  let configSection = '';
  if (configFile && fileExists(configFile)) {
    const cfg = readJSON(configFile);
    if (cfg) configSection = `## Project Config\n\`\`\`json\n${JSON.stringify(cfg, null, 2)}\n\`\`\`\n`;
 ...
 */
function buildDeepPlanCondensePrompt(description, discoveryContent, analysisContent, configFile, projectRoot, draftPath, srcPath = null) {
  let completedSection = '';
  const allTasks = getAllTasks(projectRoot);
  const done = allTasks.tasks.filter(t => t.status === 'completed');
  if (done.length > 0) {
