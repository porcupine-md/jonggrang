...
function updateTask(tasksFile, taskId, updates) {
  ...
  const UPDATABLE = new Set([
    'title', 'description', 'priority', 'status', 'role', 'skill',
...
7. Mark task done: run \`jonggrang task done ${taskId}\`
...
Write the result to \`.jonggrang/jonggrang-tasks.json\` using this EXACT schema:

\`\`\`json
{
  "feature": "from-plan-frontmatter-feature-field",
  "branch": "from-plan-frontmatter-branch-field",
  "tasks": [
    {
      "id": "task-001",
...
## Rules
- feature and branch must match the plan's frontmatter exactly
...
- Use blocked_by to encode phase dependencies
- priority 1 = first to execute, 2 = next, etc.
- Cover ALL phases — do not skip any phase from the plan
- After writing, report a brief summary: how many tasks per phase`;
}
