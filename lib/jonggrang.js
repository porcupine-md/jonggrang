 ...
 */
function buildDraftPlanPrompt(description, configFile, projectRoot) {
  let configSection = '';
 ...
 */
function buildDeepPlanCondensePrompt(description, discoveryContent, analysisContent, configFile, projectRoot) {
  let completedSection = '';
  const allTasks = getAllTasks(projectRoot);
  const done = allTasks.tasks.filter(t => t.status === 'completed');
  if (done.length > 0) {
