...
function recall(projectRoot, { phase, query, featureId, taskId } = {}) {
  const snippets = [];
  const sources = [];
...
// Returns the memory policy + mandatory recall guide for a phase.
// Injected into plan/approve/work/review prompts. NEVER dumps full memory —
// the agent must call `jonggrang memory recall ...` to fetch bounded snippets.
const PHASE_RECALL_GUIDE = {
  plan:    'jonggrang memory recall --phase plan --query "<feature goal>"',
  approve: 'jonggrang memory recall --phase approve --query "<decomposition>" [--feature <id>]',
  work:    'jonggrang memory recall --phase work --feature <feature_id> --task <task_id> --query "<task goal>"',
  review:  'jonggrang memory recall --phase review --feature <feature_id> --query "<review focus>"',
  simplify:'jonggrang memory recall --phase simplify --feature <feature_id> --query "<simplification target>"',
  test:    'jonggrang memory recall --phase test --feature <feature_id> --query "<test focus>"',
};

function buildRecallGuide(phase, { featureId, taskId } = {}) {
  const fid = featureId || '<feature_id>';
  const tid = taskId || '<task_id>';
  switch (phase) {
    case 'plan':
      return 'jonggrang memory recall --phase plan --query "<feature goal>"';
    case 'approve':
      return `jonggrang memory recall --phase approve --query "<decomposition>"${featureId ? ` --feature ${featureId}` : ' [--feature <id>]'}`;
    case 'work':
      return `jonggrang memory recall --phase work --feature ${fid} --task ${tid} --query "<task goal>"`;
    case 'review':
      return `jonggrang memory recall --phase review --feature ${fid} --query "<review focus>"`;
    case 'simplify':
      return `jonggrang memory recall --phase simplify --feature ${fid} --query "<simplification target>"`;
    case 'test':
      return `jonggrang memory recall --phase test --feature ${fid} --query "<test focus>"`;
    default:
      return `jonggrang memory recall --phase ${phase} --query "<goal>"`;
  }
}
...
function buildMemoryPolicyPrompt(phase, { featureId, taskId } = {}) {
  ...
  const lines = [
    ...
    '',
    `Before this phase, run:`,
    '```',
    guide,
    '```',
    'The recall output is bounded (max snippets/chars, scoped by phase/feature/task/query).',
  ];
