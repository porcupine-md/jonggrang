...
async function compact(projectRoot, featureId, { tool, permMode, agentFn } = {}) {
  validateFeatureExists(projectRoot, featureId);
  ...
  try {
    ...
    fs.writeFileSync(tmp, result.text.trim() + '\n');
    ...
    return { skipped: false, memoryFile: memFile, fragmentsArchived: fragFiles.length };
  } finally {
...
async function promote(projectRoot, featureId, { tool, permMode, agentFn } = {}) {
  validateFeatureExists(projectRoot, featureId);
  const featFile = featureMemoryFile(projectRoot, featureId);
  if (!fs.existsSync(featFile)) {
  ...
  try {
    ...
    return { skipped: false, projectMemoryFile: projFile };
  } finally {
...
function buildMemoryPolicyPrompt(phase, { featureId, taskId } = {}) {
  const guide = PHASE_RECALL_GUIDE[phase] || `jonggrang memory recall --phase ${phase} --query "<goal>"`;
  const lines = [
